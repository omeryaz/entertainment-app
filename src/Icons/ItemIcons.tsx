...
const itemIconClass = "inline-block text-center";

export function ItemMovieIcon() {
  return (
    <svg
...
export function ItemSeriesIcon() {
  return (
    <svg
      className={itemIconClass}
      width="12"
...
export const ItemBookmarkIcon = ({
  title,
  isBookmarked,
}: ItemBookmarkIconProps) => {
  const { setBookmarkItems } = useGeneral();
  const toggleBookmark = () => {
    setBookmarkItems((prevState) => {
      const updatedBookmarks = { ...prevState };
      updatedBookmarks[title] = !updatedBookmarks[title];
      return updatedBookmarks;
    });
  };
  return (
    <button onClick={() => toggleBookmark()} className={bookmarkIconClass}>
      {isBookmarked ? (
